...
export const STATUS_THEMES: Record<Budget['status'], { label: string; bg: string; text: string; border: string }> = {
  draft: { label: 'Rascunho', bg: 'bg-muted/60', text: 'text-muted-foreground', border: 'border-input/60' },
  ...
  expired: { label: 'Expirado', bg: 'bg-zinc-800/40', text: 'text-zinc-400', border: 'border-zinc-800/60' },
};
...
export default function StatusBadge({ status, onClick }: StatusBadgeProps) {
  ...
  const className = `inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold uppercase tracking-wider border ${theme.bg} ${theme.text} ${theme.border}${
